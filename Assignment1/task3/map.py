...
for line in sys.stdin:
	...
	# if this is the first row of attributes
        if info[0] == 'medallion':
            continue
        keys = list(info[i] for i in trip_index)
	# TODO
        values = list(info[i] for i in trip_content) 
	#print '%s\t%s' % (','.join(keys), ','.join(values))
	
	keys = ','.join(keys)
	values = ','.join(values)
	if trip_records.get(keys) == None:
		trip_records[keys] = []
	trip_records[keys].append(values)
	...
	# TODO
	values = list(info[i] for i in fare_content)
        #print '%s\t%s' % (','.join(keys), ','.join(values))

	keys = ','.join(keys)
        values = ','.join(values)
        if fare_records.get(keys) == None:
                fare_records[keys] = []
        fare_records[keys].append(values)

for key in trip_records:
	try:
		trip_values = trip_records[key]
		fare_values = fare_records[key]
		for i in trip_values:
			for j in fare_values:
				print '%s\t%s' % (key, i + ',' + j)		
